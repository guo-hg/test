...
def test_3(lian):
    ...
    assert a2 == "编辑资料"
    log.info("成功进入编辑资料页面")
    lian.find_elements_by_class_name("android.widget.ImageView")[-2].click()
    sleep(5)
    lian.find_element_by_id("com.qk.butterfly:id/tv_right").click()   # 删除照片
    sleep(3)
    lian.find_element_by_id("com.qk.butterfly:id/tv_top_right").click()   # 保存
    sleep(5)
    log.info("删除照片成功")
# # 测试四：向上滑动，进入
def test_4(lian):
