...
# # SFTPClient.from_transport(t)     sftp客户端方法         参数  指的是一个套字节服务端口   ————t
...
import paramiko
class As(object):
    t = paramiko.Transport("192.168.10.43", 22)         #  连接linux系统
    t.connect(username="root", password="123456")
    def __init__(self,name1,name2):                 #  使用init方法
        self.name1 = name1
        self.name2 = name2
    def sd(self):
        sftp = paramiko.SFTPClient.from_transport(self.t)       #创建文件传输通道
        x1 = self.name1      # linux远程文件
        x2 = self.name2
        sftp.get(x1,x2)
        self.t.close()
a = As("/home/python.py",r"E:\untitled1\python\B\aa.txt")
a.sd()
...
#     def c(self):
#         #   exec_command(需要执行的命令)：  执行命令发的方法，命令要写成字符串类型
#         stdin,stdout,stderr = self.s.exec_command("source 99.sh")   #stdin  命令， stdout 结果，  stderr   错误
#         print(stdout.read().decode())       #  读取需要执行命令的结果
# m = B()
# m.c()
